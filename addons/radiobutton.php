<?php $first = true; foreach ( $addon['options'] as $i => $option ) :
	...
	<div class="choice-box__row">
		<div class="choice-box__row-radio">
			<div>
				<label>
					<input type="radio" class="addon addon-radio" name="addon-<?php echo sanitize_title( $addon['field-name'] ); ?>[]" data-price="<?php echo $option['price']; ?>" value="<?php echo sanitize_title( $option['label'] ); ?>" <?php checked( $current_value, 1 ); ?> /> <?php echo wptexturize( $option['label'] . ' ' . $price ); ?>
				</label>
			</div>
		</div>
	</div>
