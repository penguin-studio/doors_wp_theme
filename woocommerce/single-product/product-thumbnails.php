...
if ( $attachment_ids ) {
	$loop 		= 0;
	$columns 	= 3;
	$img_count  = count($product->get_gallery_attachment_ids());
	?>
	...
	<?php
		...
		foreach ( $attachment_ids as $attachment_id ) {
			...
			$image       = wp_get_attachment_image( $attachment_id, apply_filters( 'single_product_small_thumbnail_size', array(87,130) ), 0, $attr = array(
				'title'	=> $image_title,
				'alt'	=> $image_title
				) );
			...
			$loop++;
			if ( $loop % $columns === 0 || $img_count == $loop)
